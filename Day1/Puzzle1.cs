...
class Puzzle1 : IPuzzle
{
    ...
    private string[] digits = new[] { "zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine" };
    ...
    private int Part2(string line)
    {
        ...
        {
            if (TryParseDigit(line, i, out right))
            {
                break;
            };
        }
        ...
        {
            if (TryParseDigit(line, i, out left))
            {
                break;
            };
        }
    ...
    private bool TryParseDigit(string line, int start, out int digit)
    {
        ...
        for (int d = 1; d < 10; d++)
        {
            var digitName = digits[d];
            if (start + digitName.Length <= line.Length)
            {
                if (line.Substring(start, digitName.Length) == digitName)
                {
                    digit = d;
                    return true;
                }
    ...
    int Part1(string line)
    {
        ...
        for (int i = 0; i < line.Length; i++)
        {
            ...
            if (char.IsNumber(line[i]) && left < 0)
            {
                left = line[i] - 48;
            }

            if (char.IsNumber(line[line.Length - i - 1]) && right < 0)
            {
                right = line[line.Length - i - 1] - 48;
            }
